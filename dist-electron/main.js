...
function unlockDatabase(password, isRecovery = false) {
	try {
		...
		const userKey = deriveUserKey(password, Buffer.from(meta.salt, "base64"));
...
function changePassword(oldPassword, newPassword) {
	try {
		const metaStr = fs.readFileSync(META_FILE, "utf-8");
		const meta = JSON.parse(metaStr);
		const oldUserKey = deriveUserKey(oldPassword, Buffer.from(meta.salt, "base64"));
...
ipcMain.handle("get-summary", async () => {
	try {
		...
		const overdueCount = await prisma.checkout.count({ where: {
			status: "ACTIVE",
...
ipcMain.handle("return-checkout", async (_, { id, conditionIn }) => {
	try {
		...
		const res = await prisma.$transaction(async (tx) => {
			...
			if (subject) {
				const returnedCheckouts = subject.checkouts.filter((c) => c.status === "RETURNED" && c.conditionIn !== null);
				let totalDegradation = 0;
				returnedCheckouts.forEach((c) => {
