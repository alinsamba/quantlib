...
n.handle("add-incident", async (e, t) => {
	...
	try {
		...
		let e = await N.$transaction(async (e) => {
			...
			if (i) {
				let t = i.checkouts.filter((e) => e.status === "RETURNED" && e.conditionIn !== null), a = 0;
				t.forEach((e) => {
					a += e.conditionOut - (e.conditionIn || e.conditionOut);
				});
				let o = t.length > 0 ? a / t.length : 0, s = i.openingCount + i.recovered - i.lost - i.damaged, c = r.conditionOut - (n || r.conditionOut), l = s > 0 ? c / s : c, u = Math.max(1, i.averageCondition - l);
