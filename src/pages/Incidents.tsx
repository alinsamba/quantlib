  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-800">Incident Log</h1>
                ...
                <tr key={inc.id} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                      inc.type === 'DAMAGED' ? 'bg-amber-100 text-amber-700' :
                      inc.type === 'LOST' ? 'bg-red-100 text-red-700' :
                  ...
                  <td className="p-4 font-medium">{inc.bookTitle}</td>
                  <td className="p-4">{inc.responsibleParty || '-'}</td>
                  <td className="p-4">{inc.studentClass || '-'}</td>
                  <td className="p-4 text-slate-500">{inc.condition || '-'}</td>
                </tr>
              ))}
              {incidents.length === 0 && (
                 <tr>
                   <td colSpan={7} className="p-8 text-center text-slate-500">No incidents logged yet</td>
                 </tr>
              )}
            </tbody>
          </table>
